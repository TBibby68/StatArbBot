...
def analyse_window(args):
    """
    Run adf tests for every stock within one training window.
    ...
    window = PRICE_DATA[start:end] # this needs to be log prices!
    ...
    for i in range(number_of_stocks):
        ...
        # ADF on level series
        level_result = adfuller(series)
        level_p_value = level_result[1]
        ...
        results.append(
            {
                "window_id": window_id,
                "stock": stock,
                "level_p_value": level_p_value,
                "diff_p_value": diff_p_value,
                "passes_i1": passes_i1_check,
            }
        )
...
def main():
    ...
    backtesting_data = pd.read_sql(
        "SELECT * FROM backtesting_data_energy",
        con=engine
    ...
    print("Results written to adf_results_energy.")

    # Optional verification
    df_check = pd.read_sql(
        """
        SELECT *
        FROM adf_results_energy
